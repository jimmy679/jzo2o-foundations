...
public class RegionServiceImpl extends ServiceImpl<RegionMapper, Region> implements IRegionService {
    ...
    @Resource
    private IServeItemService iServeItemService;
    ...
    public void active(Long id) {
        ...
        Integer count = baseMapper.queryByRegionId(id);
        if(count == 0){
            throw new ForbiddenOperationException("该区域下没有上架的服务，不能启用！");
        }
    ...
    public void deactivate(Long id) {
        ...
        Integer count = baseMapper.queryByRegionId(id);
        if(count > 0){
            throw new ForbiddenOperationException("该区域下有上架的服务，不能禁用！");
        }
